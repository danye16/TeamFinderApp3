...
const AuthModal: React.FC<AuthModalProps> = ({ isVisible, onClose, onLoginSuccess }) => {
 ...
 if (isLoginMode) {
      ...
      // --- ¡LA MAGIA FINAL! ---
      const finalRegisterData: UsuarioCreacionDto = {
        ...baseRegisterData,
        steamId: "", // <-- Enviamos una cadena vacía, no null
        estiloJuego: "Competitivo", // <-- Enviamos el valor por defecto
      };
      
      const result = await handleRegister(finalRegisterData);
      if (result.success) {
        onLoginSuccess();
      } else {
        alert(result.error);
